       ...
       //console.log(item);
        ...
        //if (asset) {
            ...
            if (assetFeaturesStatus && assetFeaturesStatus.stats) {
                ...
                ret +=                  '<div class="col-50">';
                ret +=                     '<i class="f7-icons icon-other-data-battery-level-1 asset_list_icon color-dealer"></i>';                 
                ...
                ret +=  '<li class="item-link item-content item_asset" data-imei="' + item.IMEI + '" data-id="' + item.Id + '" data-imei="' + item.Name + '" title="No data">';                    
                ret +=      '<div class="item-media">'+assetImg+'</div>';
                ret +=      '<div class="item-inner">';
                ret +=          '<div class="item-title-row">';
                ret +=              '<div class="item-title">' + item.Name + '</div>';
    ...
    if (params && imgFor.assetList) {        
        if (params.Icon && pattern.test(params.Icon)) {
            ...
            if (splitted.length > 1) {
                assetImg = '<div class="item_asset_img bg-dealer"><div class="text-a-c vertical-center user_f_l fs-24 color-white">'+splitted[0][0]+splitted[1][0]+'</div></div>';            
            }else{
...
function loadPageRechargeCredit(){
    ...
    var buttons = {
        'button10' : '4UGU3DZN9SEMC',
        'button50' : 'G85YFA84VV69Y',
        'button100' : 'FPNPB43PHCCLW',
        'buttonCur' : 'CAD' 
    };  

    switch (CountryCode){
        case 'USA':
            buttons.button10  = 'K3ATFQYKRJAAA';
            buttons.button50  = 'KUQKPEHFD2GR6';
            buttons.button100 = '6F34CC6NV9YW8';
...
function loadPageSupport(){
	...
	var param = {
		...
		'email':'',
		'phone':'',
        'service':'22', //means quikloc8.co in support page
	};
...
function updateTrackingHint(params){        
    ...
    params.upgradeNowButtonEl.data('paylink',Protocol.TrackingInterval[params.value][trackingPeriodElVal].button[params.currency]+'&on0=imei&os0='+params.assetImei+'&on1=dn&os1='+AppDetails.code+'&on2=appname&os2='+AppDetails.name);
